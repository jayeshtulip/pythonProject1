...
# Wait for the load balancer to be in the active state
elbv2.wait_until_load_balancer_available(LoadBalancerArn=load_balancer_arn)

# Send HTTP traffic to the instances through the load balancer
response = requests.get(f'http://{load_balancer_dns_name}')
print(response.text)
